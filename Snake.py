...
tela = pygme.display.set_mode((dimensoes))
...
def desenha_cobra(lista_cobra):
    ...
    for unidade in lista_cobra:
        pygame.draw.rect(tela, orange, [unidade[0], unidade[1], d, d])
...
def mover_cobra(dx, dy, lista_cobra):
    ...
    x = lista_cobra[-1][0] + dx
    y = lista_cobra[-1][1] + dy
...
def verifica_cauda(lista_cobra):
    head = lista_cobra[-1]
...
def atualizar_pontos(lista_cobra):
    puts = str(len(lista_cobra))
    score = font.render("Score: " + pts, True, amarelo)
    tela.blit(value [ 0, 0])
